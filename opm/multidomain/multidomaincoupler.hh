...
template <class TypeTag>
class DarcyCoupler {
    typedef typename GET_PROP_TYPE(TypeTag, Coupler) Implementation;
    typedef typename GET_PROP_TYPE(TypeTag, Scalar) Scalar;
    ...
    typedef Dune::FieldMatrix<Evaluation, dimWorld, dimWorld> EvalDimMatrix;
...
public:
    ...
    void volumeFlux(const CouplingElementContext& elemCtx)
    {
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            ...
            for (unsigned dimIdx = 0; dimIdx < dimWorld; ++dimIdx) {
                Scalar tmp = exteriorPos[dimIdx] - interiorPos[dimIdx];
                quantityGrad[dimIdx] = (tmp / distSquared) * (p1 - p0); //deltay * (tmp / distSquared);
            }

            Evaluation K0 = 0.0;
            Evaluation K1 = 0.0;
            const auto K0mat = elemCtx.template intensiveQuantities<0>(0, /*timeIdx=*/0).intrinsicPermeability();
            const auto K1mat = elemCtx.template intensiveQuantities<1>(0, /*timeIdx=*/0).intrinsicPermeability();
            for (unsigned dimIdx = 0; dimIdx < faceNormal.size(); dimIdx++) {
            ...
            Evaluation Kr0;
            Evaluation Kr1;
            const auto& materialParams0 = simulator0_.problem().materialLawParams(elemCtx.template subElementContext<0>(), 0, 0);
            const auto& materialParams1 = simulator1_.problem().materialLawParams(elemCtx.template subElementContext<1>(), 0, 0);
            if (upstreamDofIdx == 0) {
                Kr0 = elemCtx.template intensiveQuantities<0>(0, /*timeIdx=*/0).relativePermeability(phaseIdx);
                // const auto& fluidState = elemCtx.template intensiveQuantities<0>(0, /*timeIdx=*/0).fluidState();
                // EvalDimVector saturations;
                // MaterialLaw<0>::saturations(saturations, materialParams0, fluidState);
                // if (phaseIdx == MaterialLaw<0>::Traits::wettingPhaseIdx) {
                //     Kr1 = MaterialLaw<1>::twoPhaseSatKrw(materialParams1, saturations[MaterialLaw<1>::Traits::wettingPhaseIdx]);
                //     //Kr0 = MaterialLaw<0>::twoPhaseSatKrw(materialParams0, saturations[MaterialLaw<0>::Traits::wettingPhaseIdx]);
                // } else if (phaseIdx == MaterialLaw<0>::Traits::nonWettingPhaseIdx) {
                //     Kr1 = MaterialLaw<1>::twoPhaseSatKrn(materialParams1, saturations[MaterialLaw<1>::Traits::wettingPhaseIdx]);
                //     //Kr0 = MaterialLaw<0>::twoPhaseSatKrn(materialParams0, saturations[MaterialLaw<0>::Traits::wettingPhaseIdx]);
                // } else
                ...
                Kr1 = elemCtx.template intensiveQuantities<1>(0, /*timeIdx=*/0).relativePermeability(phaseIdx);
                // const auto& fluidState = elemCtx.template intensiveQuantities<1>(0, /*timeIdx=*/0).fluidState();
                // EvalDimVector saturations;
                // MaterialLaw<0>::saturations(saturations, materialParams0, fluidState);
                // if (phaseIdx == MaterialLaw<0>::Traits::wettingPhaseIdx) {
                //     //Kr1 = MaterialLaw<1>::twoPhaseSatKrw(materialParams1, saturations[MaterialLaw<1>::Traits::wettingPhaseIdx]);
            ...
            if (false && Grid<0>::dimension == Grid<1>::dimension && Grid<0>::dimension != 1) {
            ...
            if (upstreamDofIdx == 0) {
                if (static_cast<int>(focusDofIdx) == 0) {
                    ...
                    viscosity = elemCtx.template intensiveQuantities<0>(0, /*timeIdx=*/0).fluidState().viscosity(phaseIdx);
                    ...
                    viscosity = Opm::getValue(elemCtx.template intensiveQuantities<0>(0, /*timeIdx=*/0).fluidState().viscosity(phaseIdx));
                }
            } else {
                if (static_cast<int>(focusDofIdx) == 1) {
                    ...
                    viscosity = elemCtx.template intensiveQuantities<1>(0, /*timeIdx=*/0).fluidState().viscosity(phaseIdx);
                } else {
                    ...
                    viscosity = Opm::getValue(elemCtx.template intensiveQuantities<1>(0, /*timeIdx=*/0).fluidState().viscosity(phaseIdx));
                }
            }
